...
- **U15 · Record re-home — verb + pane proposability** [U12] *(added
  2026-07-18 — feedback round 3 item 3; 09 §4.5/§11 Y-17; 02 §2 verb
  pin; routing-doctrine + pane-surface-model amendments of the same
  date. U14 is left free for the round-3 items-1/2 author working in
  parallel; the orchestrator resolves numbering at merge)*: the CLI
  verb `rehome <id> --to <path-or-slug>` (project→project only;
  pending/deferred **status** check, never mere existence;
  registered-target gate reading hosts.yaml; target bucket dirs +
  `meta.yaml` created if absent; proposal siblings swept via `git rm`,
  never moved; one ledger commit `self-learn: rehome lrn-… →
  projects/<slug>`, `--note` → commit body; standard
  lock/sentinel/scan/push sequence); `propose_verb` gains `to`
  ...
  line "move this lesson to the ⟨name⟩ project", resolved path as
  trailing metadata, target = server truth); the slot clear-set's
  bucket-change leg; **no human-side re-home key or control in M1**
  (agent-proposal + CLI parity — dated choice, Y-17 decision 3).
  *Tests:* cli-suite verb tests on a constructed two-project ledger
  (move + deferred-record move, bucket creation + meta.yaml stamp,
  sibling sweep, each refusal string verbatim, commit subject,
  re-run refuses on target==current); ui-suite `validate_proposal`
  cases (happy path + every intake refusal) + shimmed-runner confirm
  asserting argv `rehome <id> --to <path>` + a clear-set test for a
  CLI-side rehome during a waiting proposal. *DoD:* cli + ui suites
