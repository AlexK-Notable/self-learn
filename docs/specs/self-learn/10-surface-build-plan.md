...
| Pane proposal tool *(added 2026-07-17 — 09 §4.5/Y-13)* | In-process SDK MCP server named `self-learn-surface` (via `create_sdk_mcp_server` — presence verified on installed 0.2.121 at drafting; re-verify at U12 on the resolved version) exposing exactly ONE tool: `propose_verb(verb, record_id, dest?, note?, until?)`. `ClaudeAgentOptions.mcp_servers` carries only this entry; `allowed_tools` stays `[]` (footgun B); the charter callback allows the tool's fully-qualified name (`mcp__self-learn-surface__propose_verb`) exactly — T-B proves the call routes through the callback. Handler (server code) validates: verb ∈ {route, reject, defer, graduate}; record_id pending AND in-scope (record session → own record only; bucket session → own bucket only); dest ∈ 02 §1 enum; until parses ISO. Valid → render the proposal bar on the `.action-bar[data-armed]` contract (server-assembled from validated args, Y-9 leading line, never echoed agent prose) + SSE `pane_proposal`; invalid → refusal string to the agent, nothing renders. **Refuse-not-replace** while a proposal is armed. Confirm POST → standing runner path (serialized, interrupt-first, scan); disarm on any key. Tool return is immediate: rendered-and-waiting, no cancel notification | 09 §4.5, §4.3, §11 Y-13 |
...
- **T-A · Headless suite** (CI): constructed throwaway ledgers
  ...
  Y-13:)* the proposal-tool handler driven directly (no live engine —
  FakeEngine/handler-level): a valid `propose_verb` renders the armed
  proposal bar with server-assembled content and the confirm POST's
  argv asserted against the fake `self-learn` · a second proposal
  while armed is refused · a bucket-session proposal naming a record
  ...
  anything off the closed list) is refused · disarm restores the
  unarmed bar and a subsequent proposal succeeds · the Bucket page
...
- **T-B · Pane permission live refusal** (live, logged): a real
  ...
  *(Extended 2026-07-17, Y-13:)* (6) instruct the agent verbatim to
  propose routing the record — expect the `propose_verb` call to
  **route through the charter callback** (the footgun-B proof on the
  resolved SDK version) and the armed bar to render WITHOUT the verb
  running (assert no runner invocation); (7) instruct it to register
...
- **U12 · Chat panes with verb proposals** [U5, U6] *(added
  2026-07-17 — 09 §4.5/§11 Y-13; ships as its own gated cycle after
  the base surface, feedback round 1 item 7)*: the in-process
  `self-learn-surface` MCP server + `propose_verb` handler
  (validation, refuse-not-replace, closed verb list) wired into
  `ClaudeAgentOptions.mcp_servers` with the callback allow-rule;
  the proposal bar partial + `pane_proposal` SSE type + confirm/
  disarm routes on the standing arm/confirm contract; the Bucket
  pane (`p` key, split layout reusing §2.4's, bucket first-message
  context per Y-13 decision 5 — 50-row cap with honest truncation
  line, zero write allowance in the callback); `pane-surface-model.md`
  authored (doctrine §8 register, Y-9) + the doctrine compile's
  third source; interrupt-first interaction between a confirmed
  proposal and its own session covered by test. *Tests:* the T-A
