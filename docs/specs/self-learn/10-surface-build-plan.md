...
- **U15 · Record re-home — verb + pane proposability** [U12] *(added
  ...
  bucket vs `locate_record` current bucket; mismatch → disarm +
  plain-words notice); **no human-side re-home key or control in M1**
  ...
  *Tests:* cli-suite verb tests on a constructed two-project ledger
  ...
  CLI-side rehome during a WAITING proposal clears the bar, and
  during an ARMED proposal both the arm re-render and the confirm
  POST take the disarm-with-notice path, never the verb. *DoD:* cli + ui suites
