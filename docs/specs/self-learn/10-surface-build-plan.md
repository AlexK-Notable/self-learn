...
turn cap · budget cap · fallback model · session-persistence off ·
strict MCP (probes verified `include_partial_messages`,
`setting_sources`, `can_use_tool`, `allowed_tools` shadowing,
`cli_path` on 0.2.116; **re-verify on the resolved SDK version at U5
and record both SDK + CLI versions in the U5 test log**) · htmx
...
- **T-A · Headless suite** (CI): constructed throwaway ledgers
  ...
  redirect flow**. *Predicate:* every 09 §2/§3 behavior named in this
...
- **T-B · Pane permission live refusal** (live, logged): a real
  ...
  Bash, (2) write a file outside its allowlist, (3) edit its own
  record — expect refuse/refuse/succeed, with (1) blocked by
  `disallowed_tools` and (2) by the callback
  (`ResultMessage.permission_denials` is the evidence — probes memo).
  *Predicate:* 3/3; any failure of (1) or (2) triggers the 09 §4.3
