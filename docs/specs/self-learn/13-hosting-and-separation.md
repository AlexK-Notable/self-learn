...
- `hosts.yaml` is data, tracked in the ledger repo — one file to read
  ...
  *(Amended 2026-07-18 — feedback round 3 item 2; normative text at
  09 §11 Y-17, this bullet is its CLI-side mirror.)* `host add`
  gains `--init`: `git init` + an empty root commit (pinned subject
  `self-learn: init for host registration`) at the EXACT path,
  performed before the existing validation, which then runs
  unchanged. Semantics per the Y-17 matrix: no-op when the path is
  already a repo root; init-at-the-exact-path when it is not —
  including when a parent repo's work tree swallows it (nested
  repos are acceptable and intended); refusal on a missing dir
  (`--init` never creates directories); all other refusals and the
