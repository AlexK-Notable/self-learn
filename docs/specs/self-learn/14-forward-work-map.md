...
| FW-51 | **`graduate` on a REJECTED record succeeds, silently rewriting a denied lesson to `superseded_by: canon`.** Unguarded in the CLI — measured in a throwaway sandbox 2026-08-02, not inferred. `graduate` on an already-graduated record fails safe (nothing to commit); on a rejected one it goes through and inverts the human's decision | BUILD | Independent of the campaign; `verbs.py` is contended, so sequence it after. Not reachable through `U-grad-ui`'s surface (which gates the action to `routed`), so this is a CLI-side hole only — but the CLI is the path a human is most likely to take by hand. Found 2026-08-02 while spec'ing `U-grad-ui` |
