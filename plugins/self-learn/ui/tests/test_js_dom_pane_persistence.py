...
# (mirrors test_js_dom.py's identical block). `conftest.py`'s
# `_browser_gate`/`_floor_playwright_browsers_path` reassert the
# identical value at fixture time (U-browserfail) -- this module-level
# copy stays, since it is what makes THIS constant correct even when
# this module is the ONLY one of the Playwright trio collected.
