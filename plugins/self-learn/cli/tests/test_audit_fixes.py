...
class Env:
    def __init__(self, tmp_path):
        ...
        (self.skill_dir / "SKILL.md").write_text("# s\n", encoding="utf-8")
...
def test_teach_env_value_secret_refused_nothing_written(env, capsys):
    ...
    pending = env.skill_dir / ".self-learn" / "pending"
...
def test_teach_route_env_secret_refused_nothing_committed(env, capsys):
    ...
    resolved = env.skill_dir / ".self-learn" / "resolved"
...
def test_multi_file_flush_is_all_or_nothing(env):
    ...
    # NOTHING moved — including the clean June file
...
def test_set_follow_up_refuses_non_routed_status(env):
    ...
    record = Record.from_path(
        env.skill_dir / ".self-learn" / "resolved" / f"{rid}.md"
    )
...
def test_fallback_capture_emits_event(env):
    ...
    rc = _capture_to_pending(env.home, record.to_text(), "boom", "retry later")
    assert rc == 4
    spooled = [
        json.loads(ln)
