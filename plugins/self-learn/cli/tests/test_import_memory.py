...
def project_bucket(home: Path):
    (b,) = [b for b in discover_buckets(home) if b.scope == "project+user"]
    return b
...
def record_of(home: Path, rid: str) -> Record:
    bucket = project_bucket(home).path
    for sub in ("pending", "resolved"):
        path = bucket / sub / f"{rid}.md"
        if path.is_file():
...
def test_one_record_per_topic_file_index_excluded(tmp_path):
    home, memory_dir = setup(tmp_path)
...
def test_fact_comes_from_description_frontmatter(tmp_path):
    ...
    # triage-visible with origin preserved (exit e)
    entries = queue(project_bucket(home))
    assert {e.record.id for e in entries} == set(report.created)


def test_idempotent_reimport_zero(tmp_path):
    home, memory_dir = setup(tmp_path)
    first = import_memory(home, memory_dir)
    second = import_memory(home, memory_dir)
...
def test_rejected_memory_never_resurrects(tmp_path):
    ...
    again = import_memory(home, memory_dir)
...
def test_scan_refused_memory_file_skipped(tmp_path):
    ...
    pending = project_bucket(home).path / "pending"
