...
def _next_run_scripts(claude_shim, monkeypatch, *scripts, exits=None, sleeps=None):
...
def test_a1_setc_tokens_pinned_both_legs(env, claude_shim):
...
def test_a5_setE_classifier_pinned_to_table_e(env, claude_shim):
...
def test_b2_world_all_valid_output(env, claude_shim, monkeypatch):
...
def test_b3_world_all_invalid_output_cleared(env, claude_shim, monkeypatch):
...
def test_b4_world_mixed_and_repair_prompt_scoped(env, claude_shim, monkeypatch):
...
def test_b5_refusal_line_matches_delete_line(env, claude_shim, monkeypatch):
    ...
    worker.run(env.home)
    prompt2 = claude_shim["call_prompt"](claude_shim["count"]())
    assert claude_shim["count"]() == base + 2  # batch, then repair
    assert f"refusal: {reason}" in prompt2


def test_b6_orphaned_record_mid_run(env, claude_shim, monkeypatch):
...
def test_b7_repair_round_times_out(env, claude_shim, monkeypatch):
...
def test_b8_world_repair_output_still_invalid(env, claude_shim, monkeypatch):
...
def test_b9_kill_switch_disables_composition(env, claude_shim, monkeypatch):
...
def test_b10_exactly_one_repair_round_never_recursive(env, claude_shim, monkeypatch):
...
def test_b11_ineligible_refusals_never_reach_a_model(env, claude_shim, monkeypatch):
    ...
    def _run_subcase(label, eligible_rid, bad_script):
        ...
        base = _next_run_scripts(claude_shim, monkeypatch, round1, round2)
        result = worker.run(env.home)
        assert claude_shim["count"]() == base + 2, label
        prompt2 = claude_shim["call_prompt"](base + 2)
...
def test_b12_repair_prompt_carries_checklist_and_rules(env, claude_shim, monkeypatch):
...
def test_b13_repair_prompt_excludes_rejudgment_materials(env, claude_shim, monkeypatch):
    ...
    _next_run_scripts(claude_shim, monkeypatch, round1)
    worker.run(env.home)
    prompt2_full = claude_shim["call_prompt"](claude_shim["count"]())
...
def test_g1_setj_pin_refuses_flipped_answer(env, claude_shim, monkeypatch):
...
def test_g2_setj_pins_positive_control(env, claude_shim, monkeypatch):
...
def test_g3_supplied_field_is_not_a_changed_field(env, claude_shim, monkeypatch):
...
def test_g4_containment_still_runs_on_repaired_output(env, claude_shim, monkeypatch):
...
def test_g5_derivation_still_runs_on_repaired_output(env, claude_shim, monkeypatch):
...
def test_g6_repair_may_not_rewrite_an_already_valid_proposal(env, claude_shim, monkeypatch):
...
def test_g7_no_ledger_mutation_between_the_invocations(env, claude_shim, monkeypatch):
    ...
    def spy_unlink(home, path, result=None):
        calls.append(("unlink", claude_shim["count"]()))
        return orig_unlink(home, path, result)

    def spy_stamp(home, record_id):
        calls.append(("stamp", claude_shim["count"]()))
...
def test_g8_sentinel_reasserted_after_the_last_invocation(env, claude_shim, monkeypatch):
    ...
    def spy_hold():
        reassert_calls.append(claude_shim["count"]())
        return orig_hold()
...
def test_d1_foreign_validated_proposal_survives_the_landing(env, claude_shim, monkeypatch):
...
def test_d2_rule_f_positive_control(env, claude_shim, monkeypatch):
...
def test_d3_rule_f_does_not_rescue_a_scan_hit(env, claude_shim, monkeypatch):
...
def test_d4_repair_round_does_not_enlarge_the_blast_radius(env, claude_shim, monkeypatch):
...
def test_d5_the_narrowed_repair_scope_is_real(env, claude_shim, monkeypatch):
...
def test_d6ii_phi_excluded_from_v(env, claude_shim, monkeypatch):
...
def test_d6iii_hook_takes_the_same_phi_branch_at_s5(env, claude_shim, monkeypatch):
...
def test_d7_a_foreign_file_counts_as_progress(env, claude_shim, monkeypatch):
...
def test_d8i_e4_batch_membership(env, claude_shim, monkeypatch):
...
def test_e5_the_backoff_suppresses_at_the_cap(env, claude_shim, monkeypatch):
...
def test_e6_an_ok_run_resets_the_counter(env, claude_shim, monkeypatch):
...
def test_f2_both_invocations_share_one_argv_builder(env, claude_shim, monkeypatch):
    ...
    worker.run(env.home)
    argv1 = claude_shim["argv"](1)
    argv2 = claude_shim["argv"](2)
...
def test_f5_shim_observes_and_drives_two_invocations(env, claude_shim, monkeypatch):
    ...
    prompt1 = claude_shim["call_prompt"](1)
    prompt2 = claude_shim["call_prompt"](2)
...
def test_h1_the_exit_code_contract(env, claude_shim, monkeypatch, capsys):
...
def test_h2_the_stdout_summary_is_byte_stable(env, claude_shim, monkeypatch, capsys):
...
def test_h3_the_five_existing_log_lines_are_byte_stable(env, claude_shim, monkeypatch):
...
def test_h4_every_new_line_in_obs1_is_produced_and_pinned(env, claude_shim, monkeypatch):
...
def test_h5_the_new_runresult_fields_are_asserted_as_fields(env, claude_shim, monkeypatch):
