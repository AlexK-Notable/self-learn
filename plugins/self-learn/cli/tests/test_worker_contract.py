...
def test_hy3_tripwire_untouched_and_live():
    """U-armor DELETE (build note): the whole-file sha check this test
    used to run against `_ARMOR_SHAS["...conftest.py"]` is retired along
    with that table -- `conftest.py`'s byte-identity is now `test_armor.
    py::test_fix1_fixtures_are_byte_identical`'s job (Fixture kind, F1).
    This test's own, non-redundant property survives: `_find_cli` is
    unreachable, tripwire-guaranteed."""
